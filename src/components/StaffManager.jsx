...
import { Mail, Smartphone, Dices, RefreshCw, UserPlus, Edit2, Check, X, Trash2, Key, ToggleLeft, ToggleRight, QrCode } from 'lucide-react'
...
export default function StaffManager({ colors, fonts, isDarkMode }) {
  ...
  const defaultStaffForm = {
    ...
    position: '', startDate: '', endDate: '', assignedCamps: [], forceReset: true
  }
  ...
  const handleStaffCreate = async (method) => {
    ...
    try {
      ...
      const { error: profileError } = await supabase
        .from('trailhead_personnel')
        .insert([{
          id: newUserId, prefix: staffForm.prefix || null, first_name: staffForm.firstName,
          middle_name: staffForm.middleName || null, last_name: staffForm.lastName,
          suffix: staffForm.suffix || null, display_name: staffForm.displayName || null,
          preferred_pronouns: staffForm.preferredPronouns || null, spoken_languages: staffForm.spokenLanguages || null,
          position: staffForm.position || null, start_date: staffForm.startDate || null,
          end_date: staffForm.endDate || null, email: staffForm.email || null, phone: staffForm.phone || null,
          is_active: true, force_password_reset: staffForm.forceReset, assigned_camps: staffForm.assignedCamps,
          trailhead_id: fullTrailheadId, access_tier: staffForm.role === 'Global Superadmin' ? 'global_superadmin' : staffForm.role === 'Global Admin' ? 'global_admin' : 'QA Tester'
        }])
  ...
  const handleStaffUpdate = async () => {
    ...
    try {
      const { error: profileError } = await supabase
        .from('trailhead_personnel')
        .update({
          prefix: staffForm.prefix || null, first_name: staffForm.firstName,
          middle_name: staffForm.middleName || null, last_name: staffForm.lastName,
          suffix: staffForm.suffix || null, display_name: staffForm.displayName || null,
          preferred_pronouns: staffForm.preferredPronouns || null, spoken_languages: staffForm.spokenLanguages || null,
          position: staffForm.position || null, start_date: staffForm.startDate || null,
          end_date: staffForm.endDate || null, email: staffForm.email || null, phone: staffForm.phone || null,
          assigned_camps: staffForm.assignedCamps, force_password_reset: staffForm.forceReset,
          trailhead_id: fullTrailheadId, access_tier: staffForm.role === 'Global Superadmin' ? 'global_superadmin' : staffForm.role === 'Global Admin' ? 'global_admin' : 'QA Tester'
  ...
  const handleToggleAccess = async () => {
    ...
    if (!selected) return
    try {
      const { error } = await supabase.from('trailhead_personnel').update({ is_active: !selected.is_active }).eq('id', selectedStaffId)
      if (error) throw error
  ...
  const openEditForm = () => {
    ...
    setStaffForm({
      prefix: selected.prefix || '', firstName: selected.first_name || '',
      middleName: selected.middle_name || '', lastName: selected.last_name || '',
      suffix: selected.suffix || '', displayName: selected.display_name || '',
      preferredPronouns: selected.preferred_pronouns || '', spokenLanguages: selected.spoken_languages || '',
      position: selected.position || '', startDate: selected.start_date || '',
      endDate: selected.end_date || '', email: selected.email || '',
      phone: selected.phone || '', assignedCamps: selected.assigned_camps || [],
      idSuffix: currentSuffix, role: parsedRole, passphrase: generatePassphrase(),
      forceReset: selected.force_password_reset || false
    })
    setIsCreatingStaff(false)
    ...
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      ...
      {showQrCode && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.8)', zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ backgroundColor: isDarkMode ? '#111' : '#fff', padding: '30px', borderRadius: '8px', maxWidth: '400px', width: '100%', border: `2px solid ${colors.primary}`, textAlign: 'center' }}>
            ...
            <button onClick={() => setShowQrCode(null)} style={{ width: '100%', backgroundColor: colors.primary, color: '#FFF', border: 'none', padding: '12px', borderRadius: '4px', cursor: 'pointer', fontFamily: fonts.header, fontSize: '16px' }}>DONE</button>
          </div>
        </div>
      )}
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
          ...
          <button onClick={() => { resetStaffForm(); setIsCreatingStaff(true); }} style={{ backgroundColor: colors.highlight, color: '#FFF', border: 'none', padding: '10px 15px', borderRadius: '4px', cursor: 'pointer', fontFamily: fonts.header, fontSize: '14px', display: 'flex', alignItems: 'center', gap: '6px' }}>
        ...
        {selectedStaffId && !isCreatingStaff && !isEditingStaff && (() => {
          ...
          return (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', marginBottom: '20px', padding: '15px', backgroundColor: 'rgba(0,0,0,0.05)', borderRadius: '4px', border: `1px solid ${colors.muted}` }}>
              <button onClick={openEditForm} style={{ backgroundColor: colors.primary, color: '#FFF', border: 'none', padding: '8px 16px', borderRadius: '4px', cursor: 'pointer', fontFamily: fonts.utility, fontSize: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}><Edit2 size={16} /> EDIT</button>
              ...
              <button onClick={handleToggleAccess} style={{ backgroundColor: isActive ? 'transparent' : colors.primary, color: isActive ? colors.textDark : '#FFF', border: `1px solid ${colors.muted}`, padding: '8px 16px', borderRadius: '4px', cursor: 'pointer', fontFamily: fonts.utility, fontSize: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                {isActive ? <ToggleRight size={16} color={colors.primary} /> : <ToggleLeft size={16} />} {isActive ? 'DISABLE' : 'ENABLE'}
              </button>
              <button onClick={handleDeleteAccount} style={{ backgroundColor: colors.error, color: 'white', border: 'none', padding: '8px 16px', borderRadius: '4px', cursor: 'pointer', fontFamily: fonts.utility, fontSize: '12px', display: 'flex', alignItems: 'center', gap: '6px', marginLeft: 'auto' }}><Trash2 size={16} /> DELETE</button>
            </div>
          )
        })()}
        ...
        {(isCreatingStaff || isEditingStaff) && (
          <div style={{ backgroundColor: isDarkMode ? '#111' : '#fff', padding: '25px', borderRadius: '4px', border: `2px solid ${colors.primary}`, marginBottom: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 style={{ fontFamily: fonts.header, fontSize: '20px', color: colors.textDark, margin: 0 }}>{isEditingStaff ? 'EDIT STAFF ACCOUNT' : 'CREATE NEW STAFF ACCOUNT'}</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginBottom: '15px' }}>
              ...
              <div style={{ padding: '15px', border: `1px solid ${colors.muted}`, borderRadius: '4px', backgroundColor: 'rgba(0,0,0,0.02)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(80px, 1fr) 2fr 2fr 2fr minmax(80px, 1fr)', gap: '10px', marginBottom: '15px', alignItems: 'end' }}>
                  <div><label style={labelStyle}>Prefix</label><select value={staffForm.prefix} onChange={(e) => setStaffForm({...staffForm, prefix: e.target.value})} style={inputStyle}><option value=""></option><option value="Mr.">Mr.</option><option value="Ms.">Ms.</option><option value="Mrs.">Mrs.</option><option value="Dr.">Dr.</option><option value="Mx.">Mx.</option></select></div>
                  <div><label style={labelStyle}>First Name</label><input type="text" value={staffForm.firstName} onChange={(e) => setStaffForm({...staffForm, firstName: e.target.value})} style={inputStyle} /></div>
                  <div><label style={labelStyle}>Middle</label><input type="text" value={staffForm.middleName} onChange={(e) => setStaffForm({...staffForm, middleName: e.target.value})} style={inputStyle} /></div>
                  <div><label style={labelStyle}>Last Name</label><input type="text" value={staffForm.lastName} onChange={(e) => setStaffForm({...staffForm, lastName: e.target.value})} style={inputStyle} /></div>
                  <div><label style={labelStyle}>Suffix</label><select value={staffForm.suffix} onChange={(e) => setStaffForm({...staffForm, suffix: e.target.value})} style={inputStyle}><option value=""></option><option value="Jr.">Jr.</option><option value="Sr.">Sr.</option><option value="II">II</option><option value="III">III</option><option value="IV">IV</option></select></div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
                  <div><label style={labelStyle}>Display Name</label><input type="text" placeholder="Optional" value={staffForm.displayName} onChange={(e) => setStaffForm({...staffForm, displayName: e.target.value})} style={inputStyle} /></div>
                  <div><label style={labelStyle}>Preferred Pronouns</label><input type="text" placeholder="They/Them, She/Her, etc." value={staffForm.preferredPronouns} onChange={(e) => setStaffForm({...staffForm, preferredPronouns: e.target.value})} style={inputStyle} /></div>
                  <div><label style={labelStyle}>Spoken Languages</label><input type="text" placeholder="English, ASL, etc." value={staffForm.spokenLanguages} onChange={(e) => setStaffForm({...staffForm, spokenLanguages: e.target.value})} style={inputStyle} /></div>
                </div>
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                <div><label style={labelStyle}>System Role</label><select value={staffForm.role} onChange={(e) => setStaffForm({...staffForm, role: e.target.value})} style={inputStyle}><option value="QA Tester">QA Tester</option><option value="Global Admin">Global Admin</option><option value="Global Superadmin">Global Superadmin</option></select></div>
                <div><label style={labelStyle}>Position / Job Title</label><input type="text" placeholder="Manager, Developer, etc." value={staffForm.position} onChange={(e) => setStaffForm({...staffForm, position: e.target.value})} style={inputStyle} /></div>
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                <div><label style={labelStyle}>Start Date</label><input type="date" value={staffForm.startDate} onChange={(e) => setStaffForm({...staffForm, startDate: e.target.value})} style={inputStyle} /></div>
                <div><label style={labelStyle}>End Date (Auto-Disable at 5PM CST)</label><input type="date" value={staffForm.endDate} onChange={(e) => setStaffForm({...staffForm, endDate: e.target.value})} style={inputStyle} /></div>
              </div>
              ...
              <div style={{ padding: '15px', border: `1px solid ${colors.muted}`, borderRadius: '4px', backgroundColor: 'rgba(0,0,0,0.02)' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', height: '180px' }}>
                  <div style={{ flex: 1, border: `1px solid ${colors.muted}`, borderRadius: '4px', overflowY: 'auto', backgroundColor: isDarkMode ? '#111' : '#fff', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ padding: '8px', backgroundColor: 'rgba(0,0,0,0.05)', fontSize: '11px', fontWeight: 'bold', textTransform: 'uppercase', textAlign: 'center', borderBottom: `1px solid ${colors.muted}` }}>Available</div>
                    <div style={{ flex: 1, padding: '5px' }}>
                      {camps.filter(c => !staffForm.assignedCamps.includes(c.id)).map(camp => (
                        <div key={camp.id} onClick={() => handleAssignCamp(camp.id)} style={{ padding: '8px 10px', fontSize: '13px', cursor: 'pointer', borderBottom: '1px solid #eee', color: colors.textDark, display: 'flex', justifyContent: 'space-between', transition: 'background 0.1s' }} onMouseOver={(e) => e.currentTarget.style.backgroundColor = 'rgba(0,0,0,0.05)'} onMouseOut={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
                  ...
                  <div style={{ flex: 1, border: `2px solid ${colors.primary}`, borderRadius: '4px', overflowY: 'auto', backgroundColor: isDarkMode ? '#111' : '#fff', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ padding: '8px', backgroundColor: colors.primary, color: 'white', fontSize: '11px', fontWeight: 'bold', textTransform: 'uppercase', textAlign: 'center' }}>Assigned</div>
                    <div style={{ flex: 1, padding: '5px' }}>
                      {camps.filter(c => staffForm.assignedCamps.includes(c.id)).map(camp => (
                        <div key={camp.id} onClick={() => handleUnassignCamp(camp.id)} style={{ padding: '8px 10px', fontSize: '13px', cursor: 'pointer', borderBottom: '1px solid #eee', color: colors.textDark, display: 'flex', justifyContent: 'space-between', transition: 'background 0.1s' }} onMouseOver={(e) => e.currentTarget.style.backgroundColor = 'rgba(193,83,27,0.1)'} onMouseOut={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <div style={{ padding: '12px', backgroundColor: 'rgba(0,0,0,0.05)', color: colors.textDark, border: `1px solid ${colors.muted}`, borderRadius: '4px', fontFamily: fonts.utility, fontWeight: 'bold', fontSize: '15px', display: 'flex', alignItems: 'center' }}>{getRolePrefix(staffForm.role)}</div>
                  <input type="text" value={staffForm.idSuffix} onChange={handleIdSuffixChange} placeholder="8 CHARACTERS" style={{ ...inputStyle, fontFamily: fonts.utility, textTransform: 'uppercase', flexGrow: 1 }} />
                  <button onClick={() => setStaffForm({ ...staffForm, idSuffix: generateRandomIdSuffix() })} style={{ padding: '0 12px', backgroundColor: 'transparent', color: colors.primary, border: `1px solid ${colors.primary}`, borderRadius: '4px', cursor: 'pointer', display: 'flex', alignItems: 'center' }} title="Generate Random ID"><Dices size={20} /></button>
                </div>
              </div>

              <div>
                <label style={labelStyle}>Passphrase Authentication</label>
                <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
                  <input type="text" value={staffForm.passphrase} onChange={(e) => setStaffForm({...staffForm, passphrase: e.target.value})} style={{ ...inputStyle, fontFamily: fonts.utility, flexGrow: 1, backgroundColor: 'rgba(0,0,0,0.02)', color: colors.primary, fontWeight: 'bold', fontSize: '14px' }} />
                  <button onClick={() => setStaffForm({ ...staffForm, passphrase: generatePassphrase() })} style={{ padding: '0 12px', backgroundColor: 'transparent', color: colors.textDark, border: `1px solid ${colors.muted}`, borderRadius: '4px', cursor: 'pointer', display: 'flex', alignItems: 'center' }} title="Regenerate Passphrase"><RefreshCw size={18} /></button>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <input type="checkbox" id="forceReset" checked={staffForm.forceReset} onChange={(e) => setStaffForm({...staffForm, forceReset: e.target.checked})} style={{ width: '16px', height: '16px', cursor: 'pointer' }} />
                  <label htmlFor="forceReset" style={{ color: colors.textDark, fontFamily: fonts.body, fontSize: '14px', cursor: 'pointer' }}>Force passphrase reset upon first login</label>
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  ...
                  {(isEditingStaff && staffForm.email) && (
                    <button onClick={() => dispatchCredentials('email')} style={{ backgroundColor: colors.highlight, color: '#FFF', border: 'none', padding: '0 20px', borderRadius: '4px', cursor: 'pointer', fontFamily: fonts.header, fontSize: '14px' }}>SEND EMAIL</button>
                  )}
                </div>
              </div>
              ...
              <div style={{ marginBottom: '10px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  ...
                  {(isEditingStaff && staffForm.phone) && (
                    <button onClick={() => dispatchCredentials('sms')} style={{ backgroundColor: colors.highlight, color: '#FFF', border: 'none', padding: '0 20px', borderRadius: '4px', cursor: 'pointer', fontFamily: fonts.header, fontSize: '14px' }}>SEND TEXT</button>
                  )}
                </div>
              </div>
              ...
              {isEditingStaff ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                   <button onClick={handleStaffUpdate} disabled={isLoading || staffForm.idSuffix.length !== 8} style={{ ...actionButtonStyle, backgroundColor: colors.primary, color: '#FFF', opacity: (isLoading || staffForm.idSuffix.length !== 8) ? 0.5 : 1 }}>
                     <Check size={18} /> SAVE CHANGES
                   </button>
                </div>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '10px' }}>
                  <button onClick={() => handleStaffCreate('email')} disabled={isLoading || staffForm.idSuffix.length !== 8} style={{ ...actionButtonStyle, backgroundColor: colors.primary, color: '#FFF', opacity: (isLoading || staffForm.idSuffix.length !== 8) ? 0.5 : 1 }}>
                    <Mail size={18} /> Email Code
                  </button>
                  <button onClick={() => handleStaffCreate('sms')} disabled={isLoading || staffForm.idSuffix.length !== 8} style={{ ...actionButtonStyle, backgroundColor: 'transparent', color: colors.textDark, border: `2px solid ${colors.textDark}`, opacity: (isLoading || staffForm.idSuffix.length !== 8) ? 0.5 : 1 }}>
                    <Smartphone size={18} /> Text Code
                  </button>
                  <button onClick={() => handleStaffCreate('qr')} disabled={isLoading || staffForm.idSuffix.length !== 8} style={{ ...actionButtonStyle, backgroundColor: colors.highlight, color: '#FFF', border: 'none', opacity: (isLoading || staffForm.idSuffix.length !== 8) ? 0.5 : 1 }}>
