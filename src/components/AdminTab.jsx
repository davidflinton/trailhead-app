  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
        ...
        <h2 style={{ fontFamily: fonts.header, fontSize: '32px', color: colors.textLight, margin: 0, letterSpacing: '1px' }}>CAMP DIRECTORY</h2>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {isLoading ? (
          <div style={{ color: colors.primary, fontFamily: fonts.utility, textAlign: 'center', padding: '20px' }}>Loading records...</div>
        ) : filtered.length === 0 ? (
          <div style={{ backgroundColor: colors.highlight, color: colors.textLight, padding: '20px', borderRadius: '4px', textAlign: 'center', fontFamily: fonts.utility, fontSize: '12px' }}>No users found for this camp.</div>
        ) : (
          filtered.map(user => {
